...
db.connect((err) => {
  if (err) {
    ...
    process.exit(1);  // Exit the process with a failed code
  }
...
// Getting Fundraiser by ID  --> WHen checking on postman choose ID 2 it has more than one donation
...
app.post('/api/fundraisers', (req, res) => {
  ...
  const values = [organizer, caption, target_funding, current_funding, city, category_id];

  db.query(query, values, (err, results) => {
      if (err) {
          console.error('Error inserting fundraiser: ', err);
          return res.status(500).json({ error: `Error while inserting fundraiser: ${err.message}` });
      }
...
app.put('/api/fundraisers/:id', (req, res) => {
  ...
  const values = [organizer, caption, target_funding, current_funding, city, active, category_id, fundraiserId];

  db.query(query, values, (err, results) => {
      if (err) {
          console.error('Error updating fundraiser: ', err);
          return res.status(500).json({ error: `Error while updating fundraiser: ${err.message}` });
      }
...
app.delete('/api/fundraisers/:id', (req, res) => {
  ...
  db.query(donationCheckQuery, [fundraiserId], (err, results) => {
      if (err) {
          ...
          return res.status(500).json({ error: `Error while checking donations: ${err.message}` });
      }
      ...
      db.query(deleteQuery, [fundraiserId], (err, results) => {
          if (err) {
              ...
              return res.status(500).json({ error: `Error while deleting fundraiser: ${err.message}` });
          }
